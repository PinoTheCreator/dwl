...
swallow(Client *c, Client *w) {
		...
		resize(c, (struct wlr_box){.x = w->geom.x, .y = w->geom.y, .width = w->geom.width, .height = w->geom.height }, 0, smartborders);
...
setfloating(Client *c, int floating)
{
	...
	if (c->isfloating && !c->bw)
		resize(c, (struct wlr_box){.x = c->geom.x, .y = c->geom.y, .width = c->geom.width, .height = c->geom.height }, 0, 1);
	arrange(c->mon);
	printstatus();
}
